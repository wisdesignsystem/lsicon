...
function getVariantComponentName(option) {
  ...
  const component = option.pathToComponent.find(
...
function getComponentStyle(componentName) {
  return (
    styleWhitelist.find((item) =>
      componentName.endsWith(tool.toFirstUpperCase(item)),
    ) || defaultStyle
  );
}
    ...
    outputters: [
      svgr({
        ...
        getComponentName: (option) => {
          ...
          let categoryItem = styleItem.children.find(
...
function getComponentsByMeta(meta) {
  const components = [];
  for (const style of meta) {
    for (const category of style.children) {
...
async function main() {
  ...
  file.writeFile(metaPath, `export default ${JSON.stringify(styles)}`);
